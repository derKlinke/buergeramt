"""
Game engine for the Schenkungssteuer adventure.
Handles game state, character interactions, and game flow.
"""
...
class GameEngine:
    ...
    def __init__(self, use_ai_characters: bool = True):
        ...
        if not getattr(client, 'api_key', None):
            print("WARNUNG: OpenAI API key nicht gefunden. Bitte setzen Sie OPENAI_API_KEY oder geben Sie --api-key an.")
            self.game_over = True
            return
        # use AI characters
        try:
            self.bureaucrats = {
                "Erstbearbeitung": ai_characters.AiHerrSchmidt(),
                "Fachprüfung": ai_characters.AiFrauMüller(),
                "Abschlussstelle": ai_characters.AiHerrWeber()
            }
    ...
    def start_game(self):
        ...
        self._print_styled("• Zeigen Sie Ausweise und Unterlagen durch Angabe von 'Personalausweis', 'Urkunde', etc.", "hint")
    ...
    def show_help(self):
        ...
        self._print_styled("In diesem Spiel versuchen Sie, eine Schenkungssteuer anzumelden. Dafür müssen Sie:", "normal")
    ...
    def _suggest_next_step(self):
        ...
        if not self.game_state.evidence_provided:
            ...
            self._print_styled("Und reichen Sie Details zur Schenkung ein: 'Ich habe eine notarielle Urkunde zur Schenkung'", "hint")
            return
        ...
        if not self.game_state.collected_documents:
            if self.game_state.current_department == "Erstbearbeitung":
                if "valid_id" in self.game_state.evidence_provided and "gift_details" in self.game_state.evidence_provided:
                    self._print_styled("Fragen Sie nach der Schenkungsanmeldung: 'Ich möchte eine Schenkungsanmeldung beantragen'", "hint")
                else:
                    missing = []
        ...
        if "Schenkungsanmeldung" in self.game_state.collected_documents and "Wertermittlung" not in self.game_state.collected_documents:
            if self.game_state.current_department != "Fachprüfung":
                ...
                if "market_comparison" not in self.game_state.evidence_provided:
                    self._print_styled("Reichen Sie eine Marktwertanalyse ein: 'Ich habe eine Marktwertanalyse'", "hint")
                ...
                if "market_comparison" in self.game_state.evidence_provided and "expert_opinion" in self.game_state.evidence_provided:
                    self._print_styled("Fragen Sie nach der Wertermittlung: 'Kann ich jetzt die Wertermittlung bekommen?'", "hint")
            return
            
        # If player has Wertermittlung but no Freibetragsbescheinigung
        if "Wertermittlung" in self.game_state.collected_documents and "Freibetragsbescheinigung" not in self.game_state.collected_documents:
            if self.game_state.current_department != "Abschlussstelle":
                ...
                if missing:
                    ...
                    self._print_styled("Fragen Sie nach der Freibetragsbescheinigung: 'Ich möchte eine Freibetragsbescheinigung'", "hint")
            return
            
        # If player has most documents but not Zahlungsaufforderung
        if "Freibetragsbescheinigung" in self.game_state.collected_documents and "Zahlungsaufforderung" not in self.game_state.collected_documents:
            if self.game_state.current_department != "Erstbearbeitung":
                self._print_styled("Gehen Sie zurück zur Erstbearbeitung: 'Ich möchte zu Herrn Schmidt'", "hint")
            else:
                self._print_styled("Fragen Sie nach der Zahlungsaufforderung: 'Ich möchte die Zahlungsaufforderung'", "hint")
            return
    ...
    def process_input(self, user_input: str) -> bool:
        ...
        if user_input_lower in ["status", "fortschritt", "dokumente", "unterlagen"]:
            docs = ", ".join(list(self.game_state.collected_documents.keys())) if self.game_state.collected_documents else "keine"
            evidence = ", ".join(list(self.game_state.evidence_provided.keys())) if self.game_state.evidence_provided else "keine"
        ...
        if self.check_win_condition():
            ...
            self._print_styled(f"Sie haben {self.game_state.attempts} Versuche gebraucht und Ihre Frustration erreichte maximal Level {self.game_state.frustration_level}.", "italic")
            self._print_styled("Sie dürfen jetzt den Brief mit dem Steuerbescheid in 4-6 Wochen erwarten.", "bureaucrat")
            self.game_over = True
    ...
    def _post_process_ai_response(self, user_input: str, response: str):
        ...
        if self.game_state.current_department == "Abschlussstelle" and "nicht verwandt" in user_input_lower:
            ...
            if ("Schenkungsanmeldung" in self.game_state.collected_documents
    ...
    def _contains_evidence_terms(self, text: str) -> bool:
        ...
        evidence_terms = [
            "personalausweis", "ausweis", "reisepass", "identität", "id", 
            "urkunde", "notariell", "schenkung", "dokument", "nachweis",
            "bescheinigung", "geburtsurkunde", "gutachten", "marktwert",
            "bewertung", "steuernummer", "steuer-id", "steuerbescheid"
        ]
    ...
    def _contains_document_terms(self, text: str) -> bool:
        ...
        document_terms = [
            "formular", "antrag", "schenkungsanmeldung", "wertermittlung",
            "freibetrag", "zahlungsaufforderung", "s-100", "w-300", "f-400",
            "z-500", "schenkungssteuer", "dokument", "bescheid"
        ]
    ...
    def _contains_transfer_suggestion(self, response: str) -> bool:
        ...
        transfer_terms = [
            "andere abteilung", "erstbearbeitung", "fachprüfung", "abschlussstelle",
            "herr schmidt", "frau müller", "herr weber", "gehen sie zu", "wenden sie sich an",
            "zuständig ist", "falsches büro", "falsche abteilung", "nicht zuständig",
            "müssen sie zu"
        ]
        ...
        if requested_docs:
            for doc_name in requested_docs:
                ...
                if self.game_state.check_document_requirements(doc_name):
                    ...
                    missing = [req for req in DOCUMENTS[doc_name]["requirements"] if req not in self.game_state.evidence_provided]
    ...
    def _bureaucratic_interruption(self):
        ...
        interruptions = [
            ("Moment! Es ist jetzt Mittagspause!", "Der Beamte verschwindet plötzlich hinter einer Tür mit einem 'Pause' Schild."),
            ("Oh, das System ist gerade abgestürzt!", "Der Beamte starrt frustriert auf den Bildschirm und tippt wahllos auf der Tastatur herum."),
            ("Einen Moment, bitte. Dringendes Telefonat!", "Der Beamte nimmt ein Telefonat entgegen und ignoriert Sie für einige Minuten."),
            ("Es tut mir leid, aber Ihr Vorgang muss neu nummeriert werden.", "Der Beamte beginnt, alle Ihre Unterlagen neu zu sortieren."),
            ("Bitte warten, ich muss kurz mit meinem Vorgesetzten sprechen.", "Der Beamte verschwindet mit Ihren Unterlagen in einem Nebenzimmer.")
        ]
        ...
        for evidence_name, evidence_data in EVIDENCE.items():
            ...
            if evidence_name_lower in input_lower:
                ...
                if not form_matched:
                    ...
                    self._print_styled(f"\nWelche Art von '{evidence_name}' möchten Sie einreichen? Akzeptierte Formen: {', '.join(forms)}", "hint")
                    evidence_detected = True
    ...
    def _should_transfer_departments(self, user_input: str) -> bool:
        ...
        transfer_keywords = ["gehen zu", "wechseln", "andere abteilung", "anderen beamten", 
                             "anderes büro", "nächste stelle", "nächster", "weiter"]
        ...
        for document_name in document_names:
            ...
            if can_receive:
                ...
                if document_name == "Schenkungsanmeldung":
                    self._print_styled("Als nächstes benötigen Sie eine Wertermittlung von der Abteilung Fachprüfung.", "hint")
                elif document_name == "Wertermittlung":
                    self._print_styled("Jetzt könnten Sie eine Freibetragsbescheinigung von der Abschlussstelle beantragen.", "hint")
                elif document_name == "Freibetragsbescheinigung":
                    self._print_styled("Mit diesen Dokumenten können Sie zur Erstbearbeitung zurückkehren, um die finale Zahlungsaufforderung zu erhalten.", "hint")
            else:
                ...
                if missing:
                    ...
                    if random.random() < 0.5:
                        ...
                        if dept != self.game_state.current_department:
                            self._print_styled(f"Eigentlich ist für dieses Dokument die Abteilung {dept} zuständig.", "hint")
    ...
    def _dynamic_redirect(self):
        ...
        elif "Schenkungsanmeldung" in self.game_state.collected_documents and "Wertermittlung" not in self.game_state.collected_documents:
        ...
        elif "Wertermittlung" in self.game_state.collected_documents and "Freibetragsbescheinigung" not in self.game_state.collected_documents:
        ...
        elif "Freibetragsbescheinigung" in self.game_state.collected_documents and "Zahlungsaufforderung" not in self.game_state.collected_documents:
            if current_dept != "Erstbearbeitung":
                target_dept = "Erstbearbeitung"
    ...
    def _handle_evidence_submission(self, evidence_name: str, form: str):
        ...
        if success:
            ...
            if enabled_docs:
                ...
                dept_docs = [doc for doc in enabled_docs if DOCUMENTS[doc]["department"] == self.game_state.current_department]
                if dept_docs:
                    ...
                    doc = dept_docs[0]
                    missing = [req for req in DOCUMENTS[doc]["requirements"] if req not in self.game_state.evidence_provided]
                    if len(missing) == 1:
                    ...
                    self._print_styled(f"Mit diesem Nachweis könnten Sie bei der Abteilung {dept} das Dokument '{suggestion}' beantragen.", "hint")
    ...
    def _handle_procedure_action(self, procedure_name: str):
        ...
        if success:
            ...
            for dept in self.bureaucrats:
                if dept == PROCEDURES[procedure_name]["department"] or PROCEDURES[procedure_name]["department"] == "any":
                    correct_dept = dept
                    break
