"""
Game rules and bureaucratic system for the Schenkungssteuer adventure.
This defines the forms, requirements, and state tracking for the game.
"""
        ...
        for loop in BUREAUCRATIC_LOOPS:
            if loop["trigger"] == self.current_procedure:
                ...
                if loop["condition"] == "missing_evidence" and len(self.evidence_provided) < 3:
                    return loop
                elif loop["condition"] == "wrong_department" and self.current_department != PROCEDURES[self.current_procedure]["department"]:
                    return loop
